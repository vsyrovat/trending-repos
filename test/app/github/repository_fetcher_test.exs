defmodule App.Github.RepositoryFetcherTest do
  ...
  test "retrieve repo info" do
    mock(fn %{method: :get, url: "https://api.github.com/repos/foo/bar"} ->
      %Tesla.Env{
        status: 200,
        body: %{"id" => 123, "full_name" => "foo/bar", "owner" => %{"login" => "foo"}},
        headers: [
    ...
    assert RepositoryFetcher.fetch("foo/bar") ==
             {
               ...
               %Repo{
                 id: 123,
                 full_name: "foo/bar",
                 data: %{"id" => 123, "full_name" => "foo/bar", "owner" => %{"login" => "foo"}}
               },
  ...
  test "api limit exceeded" do
    ...
    assert RepositoryFetcher.fetch("foo/bar") ==
             {:error, :limit_exceeded, %Limit{limit: 60, remain: 0, reset_at: ~U[2019-11-26 21:17:53Z]}}
  end
  ...
  test "incorrect argument" do
    assert_raise ArgumentError, "Argument should match the pattern \"{owner}/{repo}\"", fn ->
